...
@pytest.mark.parametrize("invalid_public_file", [None, "broken.ps1", "broken.json"])
def test_release_gate_parses_only_the_validated_public_snapshot(tmp_path, invalid_public_file):
    gate = (ROOT / "scripts/release_gate.ps1").read_text(encoding="utf-8")
    start = gate.index("        $parseErrors = @()")
    assert gate.index('throw "Public snapshot validation failed."') < start
    block = gate[start:gate.index('        $distA =', start)]
    assert block.count("Get-ChildItem -LiteralPath $publicRoot -Recurse") == 2
    public = tmp_path / "public"
    public.mkdir()
    (public / "valid.ps1").write_text("Write-Output 'valid'\n", encoding="utf-8")
    (public / "valid.json").write_text("{}", encoding="utf-8")
    ignored = tmp_path / "tmp"
    ignored.mkdir()
    for directory in [ignored, *([public] if invalid_public_file else [])]:
        for name, content in [("broken.ps1", "if ("), ("broken.json", "{broken")]:
            if directory == ignored or name == invalid_public_file:
                (directory / name).write_text(content, encoding="utf-8")
    script = tmp_path / "parse-public.ps1"
    script.write_text("param([string]$publicRoot)\n$ErrorActionPreference = 'Stop'\n" + block,
                      encoding="utf-8")
    result = subprocess.run([
        "powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(script),
        "-publicRoot", str(public),
    ], cwd=tmp_path, capture_output=True, text=True, timeout=30)
    assert (result.returncode == 0) == (invalid_public_file is None), result.stdout + result.stderr
