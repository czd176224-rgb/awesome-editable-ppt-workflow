...
Release identity is fixed by `package-info.json`: version `1.2.0`, tag `v1.2.0`, workflow `awesome-word-ppt-workflow-v1`, prompt contract `consulting-page-director-v2-six-part-image2`, and policy `generate-without-refs-edit-with-confirmed-refs`. Never overwrite the tag or reuse the version.
...
Version 1.2.0 adds five whole-deck director presets, a three-step one-time confirmation UI, the confirmed presentation taskbook, Word title authority, non-contiguous page-label support, and complete native special-page rendering. It does not invent or move Word material. Existing accepted pages remain recoverable; a new project is required when the sealed project identity is incompatible.
...
4. Create annotated tag `v1.2.0` on the exact reviewed merge commit and push it.
5. The release workflow repeats the gate, builds the deterministic Windows ZIP and publishes the GitHub Release.
6. Install `awesome-editable-ppt-workflow@editable-ppt-public`, restart Codex and verify the installed plugin reports `1.2.0`.
