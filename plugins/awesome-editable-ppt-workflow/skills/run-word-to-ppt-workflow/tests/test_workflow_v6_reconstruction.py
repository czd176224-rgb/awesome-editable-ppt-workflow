...
def test_empty_frozen_plural_does_not_suppress_legacy_single_chart_fallback(
    tmp_path: Path,
) -> None:
    project = _project(tmp_path, 1)
    receipt_path = project / "04_v6/images/page_001.json"
    ...
    authority = {
        ...
        "series": [{
            "name": "计划",
            "categories": ["尽调"],
            "start_dates": ["2026-09-01"],
            "end_dates": ["2026-09-10"],
        }],
    }
    path = project / "02_v6/page_materials/page_001.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    assert request["numeric_authority"] == authority
    assert "numeric_authorities" not in request


def test_reconstruction_request_uses_frozen_plural_authorities_from_signed_page_plan(
    tmp_path: Path,
) -> None:
    project = _project(tmp_path, 1)
    receipt_path = project / "04_v6/images/page_001.json"
    ...
    receipt["page_plan"]["numeric_authorities"] = authorities
    _write_signed_receipt(project, 1, receipt)

    request = build_reconstruction_request(project, page_number=1)

    assert request["numeric_authorities"] == authorities
    assert "numeric_authority" not in request
    request_path = project / "05_v6/reconstruction_requests/page_001.json"
    first_hash = hashlib.sha256(request_path.read_bytes()).hexdigest()
    receipt["page_plan"]["numeric_authorities"][1]["series"][0]["values"][1] = 46
    _write_signed_receipt(project, 1, receipt)
    build_reconstruction_request(project, page_number=1)
    assert hashlib.sha256(request_path.read_bytes()).hexdigest() != first_hash
...
def test_finalize_and_assemble_add_fixed_layers_without_office_or_visual_qa(tmp_path: Path):
    ...
    assert report["release_status"] == "not_release_ready"
    assert report["structure_validation"]["reason"] == "presentation_structure_not_confirmed"
