...
from workflow_v6_materials import select_numeric_authorities, select_numeric_authority  # noqa: E402
...
def _production_worker(manifest_factory, calls: list[dict], post_validate=None):
    def worker(request):
        ...
        for field in ("numeric_authority", "numeric_authorities"):
            assert page_request.get(field) == accepted_request.get(field)
...
def _plural_relationship_manifest(page_request: dict) -> dict:
    authorities = page_request["numeric_authorities"]
    charts = [
        {**authority, "box_px": [100 + index * 900, 150, 700, 560]}
        for index, authority in enumerate(authorities)
    ]
    manifest = _manifest(charts[0])
    manifest["charts"] = charts
    return _with_relationship(manifest, page_request)
...
def _freeze_two_numeric_authorities(project: Path) -> list[dict]:
    charts = [
        _one_dimensional("chart-revenue", "column"),
        {
            **_one_dimensional("chart-profit", "line"),
            "title": "Profit",
            "series": [{"name": "Profit", "categories": ["A", "B"], "values": [30, 45]}],
        },
    ]
    frozen = select_numeric_authorities(charts)
    assert len(frozen) == 2
    receipt_path = project / "04_v6/images/page_001.json"
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    receipt["page_plan"]["numeric_authorities"] = frozen
    _write_signed_receipt(project, 1, receipt)
    return frozen


def test_two_frozen_numeric_authorities_reach_two_native_ppt_charts(tmp_path: Path) -> None:
    project = _project(tmp_path / "two-frozen-charts", 1)
    authorities = _freeze_two_numeric_authorities(project)
    calls: list[dict] = []

    result = reconstruct_accepted_page(
        SimpleNamespace(project_copy=project, page_number=1),
        _accepted_outcome(project),
        page_worker=_production_worker(_plural_relationship_manifest, calls),
    )

    assert calls[0]["request"]["numeric_authorities"] == authorities
    assert "numeric_authority" not in calls[0]["request"]
    charts = {
        shape.name: shape.chart
        for shape in Presentation(project / result["final_page"]).slides[0].shapes
        if shape.has_chart
    }
    assert set(charts) == {"chart-revenue", "chart-profit"}
    assert list(charts["chart-revenue"].series[0].values) == [12.0, 18.0]
    assert list(charts["chart-profit"].series[0].values) == [30.0, 45.0]


@pytest.mark.parametrize("defect", ["missing", "changed"])
def test_finalization_rejects_missing_or_changed_per_chart_authority(
    tmp_path: Path, defect: str,
) -> None:
    project = _project(tmp_path / f"plural-{defect}", 1)
    _freeze_two_numeric_authorities(project)

    def defective_manifest(page_request: dict) -> dict:
        manifest = _plural_relationship_manifest(page_request)
        if defect == "missing":
            manifest["charts"].pop()
        else:
            manifest["charts"][1]["series"][0]["values"][1] = 46
        return manifest

    with pytest.raises(ValueError, match="sealed numeric authorit"):
        reconstruct_accepted_page(
            SimpleNamespace(project_copy=project, page_number=1),
            _accepted_outcome(project),
            page_worker=_production_worker(defective_manifest, []),
        )
