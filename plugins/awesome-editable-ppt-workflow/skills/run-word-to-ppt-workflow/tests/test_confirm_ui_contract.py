...
def test_v6_recommendations_show_composition_and_one_post_freezes_it(tmp_path: Path):
    project = make_v6_project(tmp_path, page_count=3)
    ...
    assert data["composition"]["page_count"] == 3
...
def test_visual_confirmation_leaves_generation_at_actionable_materials_boundary(tmp_path: Path):
    ...
    if str(scripts) not in sys.path:
        sys.path.insert(0, str(scripts))
    from workflow_v6_contract import new_page, new_project
    from workflow_v6_image import load_validated_image_request
    ...
    with pytest.raises(ValueError, match="requires confirmed page materials"):
        load_validated_image_request(project, page_number=1)
