...
from test_awesome_pipeline import _project as _synthetic_pipeline_project  # noqa: E402
...
def test_huangshi_post_recovery_ordinary_entry_is_idempotent_without_user_files(
    tmp_path: Path,
) -> None:
    """Keep the formal acceptance suite covering ordinary use after recovery."""
    from workflow_v6_pipeline import PipelineConfiguration, PipelineDependencies, run_pages
    from workflow_v6_state import save

    project = _synthetic_pipeline_project(tmp_path, pages=1)
    ...
    report = run_pages(
        ...
        dependencies=PipelineDependencies(
            ...
            candidate_loop=lambda workspace, **_kwargs: SimpleNamespace(
                status="accepted",
                accepted={"experiment_id": workspace.experiment_id, "recovered": True},
                attempts=(), failure_problems=(), correction_count=0,
            ),
            director_invoke=lambda *_a, **_k: external_calls.append("director"),
            provider_runner=lambda *_a, **_k: external_calls.append("image2"),
            reviewer_invoke=lambda *_a, **_k: external_calls.append("review"),
        ),
    ...
    assert report.page_outcomes[1].accepted == {
        "experiment_id": recovery_id, "recovered": True,
    }
