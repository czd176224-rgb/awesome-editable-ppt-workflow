...
from complex_page_experiment.director import (
    _normalize_runtime_director_value,
...
def test_runtime_schema_represents_nested_optional_fields_as_required_nullable() -> None:
    schema = json.loads(
        (Path(__file__).resolve().parents[2] / "schemas" / "consulting_page_director_v3.schema.json")
        .read_text(encoding="utf-8")
    )
    page_plan = schema["properties"]["page_plan"]
    quantitative = schema["$defs"]["quantitativeExhibit"]

    assert set(page_plan["required"]) == set(page_plan["properties"])
    assert page_plan["properties"]["quantitative_exhibits"]["type"] == ["array", "null"]
    assert page_plan["properties"]["numeric_authorities"]["type"] == "null"
    assert set(quantitative["required"]) == set(quantitative["properties"])
    assert quantitative["properties"]["period_ref_id"]["type"] == ["string", "null"]


def test_runtime_nullable_placeholders_do_not_create_quantitative_business_authority() -> None:
    value = copy.deepcopy(_director_value())
    value["page_plan"]["quantitative_exhibits"] = None
    value["page_plan"]["numeric_authorities"] = None
