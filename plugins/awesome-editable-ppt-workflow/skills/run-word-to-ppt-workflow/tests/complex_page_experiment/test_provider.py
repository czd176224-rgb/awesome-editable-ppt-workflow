...
import complex_page_experiment.provider as provider_module
...
def test_prior_experiment_capability_with_same_page_attempt_is_not_current_recovery_state(
    provider_fixture, monkeypatch: pytest.MonkeyPatch,
):
    workspace, view, recorder, _refs = provider_fixture
    request = build_experiment_image_request(
        workspace, view, attempt=1, prompt="Current experiment prompt",
        quality="medium", selected_reference_ids=(), strategy="initial",
        previous_candidate=None,
    )
    old_output = workspace.project_copy / "04_v6/images/page_001.prior-experiment.candidate_1.png"
    old_trace = old_output.with_suffix(".trace.json")
    provider_module._publish_bridge(workspace, request, attempt=1)
    provider_module.workflow_v6_image._issue_capability(
        request, attempt=1, output=old_output, trace=old_trace,
    )
    ...
    assert result.path.name == "page_001.complex-page-provider.candidate_1.png"


@pytest.mark.parametrize("tamper", ["duplicate", "signature", "request"])
def test_current_experiment_capability_conflict_still_fails_closed(
    provider_fixture, tamper,
):
    workspace, view, recorder, _refs = provider_fixture
    request = build_experiment_image_request(
        workspace, view, attempt=1, prompt="Current experiment prompt",
    ...
    output = workspace.project_copy / "04_v6/images/page_001.complex-page-provider.candidate_1.png"
    trace = output.with_suffix(".trace.json")
    provider_module._publish_bridge(workspace, request, attempt=1)
    capability = provider_module.workflow_v6_image._issue_capability(
        request, attempt=1, output=output, trace=trace,
    )
    if tamper == "duplicate":
        provider_module.workflow_v6_image._issue_capability(
            request, attempt=1, output=output, trace=trace,
    ...
    with pytest.raises((RuntimeError, ValueError), match="multiple Provider|signature|differs"):
        run_provider_attempt(
            workspace, request, attempt=1, timeout=17, recorder=recorder,
            runner=lambda *_a, **_k: pytest.fail("invalid capability reached provider"),
        )
