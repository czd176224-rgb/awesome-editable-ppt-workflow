...
2. Open the three-step Confirm UI once:
   ...
   The structure list is source-backed: only proposed additions with a `structure:` ID may be omitted. Never delete, reorder, retitle or change the role of source pages. A second submission is rejected.
...
- Substantive titles must come from Word. Section suggestions prefer PART, 第N章 and 一、 headings; without chapter headings, propose the first body-page title as a single section and require confirmation. `--preserve-source-layout` opts out of new structure suggestions but still permits structure confirmation.
