This is an on-demand reference. Relative paths below resolve from the parent skill directory. The public entry is `run-word-to-ppt-workflow`; do not ask the user to invoke another skill or run these internal commands.
...
In this plugin, use the command only after the core workflow has produced a PPTX. Prefer structural validation, preview, issue inspection, and a narrowly scoped repair requested by the workflow or user. Do not use it to replace Image2 generation, semantic QA, object-level reconstruction, fixed-layer placement, or ordered assembly.

## Availability

OfficeCLI is optional, separately supplied, and is not bundled by this plugin.
Use this skill only when `officecli --version` already succeeds. If it is
absent, continue with the core Word-to-editable-PPT workflow; this skill must
not download or execute a remote installer.
