...
The source is the accepted body's visual authority. Object-source decisions exist to reproduce that image editably, not to redesign or reinterpret it. Preserve visible content, composition, object geometry, hierarchy, palette, spacing, visual rhythm, and major decorative elements. There are exactly two narrow exceptions. A sealed page_plan that contains primary_relationship may correct only that relationship's node identity, edge direction, and connector endpoints; a page_plan without primary_relationship supplies no reconstruction authority, and its image prompt or content records must be ignored. Each matching chart in sealed numeric_authorities (or the legacy single-chart numeric_authority) may correct only values, units, labels, any explicitly sourced period, and quantitative geometry as specified in manifest-schema.md. Match authorities by exact object_id, preserve each one independently, and never invent a missing period. The accepted image still owns composition and style; neither exception authorizes replacing the core exhibit or reading path. Do not consult Word, page-local comments, attachments, generation prompts, or reference selections after acceptance.

1. Background recognition and repair.
2. Foreground asset separation.
3. PPT native element reconstruction.

The order exists because steps 1-2 decide object sources and step 3 consumes those decisions. Define the boundaries between background, foreground, and native structure first; then write the manifest. Local reconstruction from the accepted image is the default and uses zero Image2 calls. Only if the reconstruction request explicitly contains an authorized targeted capability may that target use `editppt image reconstruct-edit`; never author a new prompt or choose a different source image.
...
Do not create a "good enough" editable draft by rebuilding only text and layout while retaining a full-page screenshot or casually clipping foreground objects without inventory and provenance. This is false progress: it may pass deterministic validation but it fails the object-source contract. Deterministic validation is a structure gate, not a waiver — `validation.json.passed=true` never makes a forbidden foreground fallback acceptable.

When a page has complex foreground visuals, first choose the smallest faithful local representation from the accepted image: native objects where practical, otherwise a bounded accepted-image region. Do not use a full-page raster to bypass editability and do not invent an emoji, text symbol, or similar-but-different replacement. If the request explicitly requires a targeted capability, prove that targeted workflow is feasible before building `manifest.json`.
...
### 1.3 Complex Backgrounds

Preserve complex backgrounds from the accepted image using local reconstruction or a bounded source-image region when native structure cannot reproduce them. A full-page raster with editable text overlaid is still forbidden. A targeted sealed `editppt image reconstruct-edit` may be used only when the reconstruction request explicitly supplies an authorized capability for that target, such as when:
...
### 2.1 Foreground Assets Default to Local Reconstruction

Represent non-text foreground visual objects from the accepted image with native PowerPoint objects or bounded accepted-image regions whenever practical. This is the normal zero-Image2 path. When the request explicitly supplies an authorized targeted capability, that target may instead be separated from the accepted page through `editppt image reconstruct-edit`, including:
...
- Every source decision follows sections 1-3: native objects and bounded accepted-image regions remain faithful to source.png, and anything carrying an explicitly authorized separation capability uses that capability rather than a similar-but-different substitute.
