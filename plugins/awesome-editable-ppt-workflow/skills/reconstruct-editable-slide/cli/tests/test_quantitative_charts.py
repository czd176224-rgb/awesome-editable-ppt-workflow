...
from workflow_v6_materials import (  # noqa: E402
    index_source_numeric_candidates,
    resolve_numeric_authorities,
    select_numeric_authority,
)
...
def test_non_time_native_chart_omits_unstated_period_without_placeholder(tmp_path: Path) -> None:
    chart = _one_dimensional("column")
    chart.pop("period")
    manifest = _manifest(chart)
    out = tmp_path / "no-period.pptx"

    write_pptx(manifest, out, tmp_path / "manifest.json")

    names = {shape.name for shape in Presentation(out).slides[0].shapes}
    assert f"{chart['name']} Period" not in names
    assert validate_pptx.quantitative_chart_readback_violations(out, [manifest]) == []


def test_source_table_resolves_two_native_charts_and_source_value_change_updates_ppt(
    tmp_path: Path,
) -> None:
    def resolved(revenue_2025: str) -> list[dict]:
        blocks = [{
            "type": "table",
            "source_block_id": "metrics-table",
            "rows": [
                ["指标", "2024", "2025", "单位", "口径"],
                ["收入", "120", revenue_2025, "万元", "经审计"],
                ["利润", "30", "45", "万元", "经审计"],
            ],
        }]
        candidates = index_source_numeric_candidates(blocks)
        cells = {
        ...
        selections = [
            {
                "chart_id": f"chart:{key}",
                "rendering_primitive": "column_bar",
                "chart_variant": "column",
                "title_ref_id": cells[(row, 0)],
                "series_groups": [{
                    "name_ref_id": cells[(row, 0)],
                    "category_ref_ids": [cells[(0, 1)], cells[(0, 2)]],
                    "value_ref_ids": [cells[(row, 1)], cells[(row, 2)]],
                    "unit_ref_id": cells[(row, 3)],
                    "basis_ref_id": cells[(row, 4)],
                }],
            }
            for key, row in (("revenue", 1), ("profit", 2))
        ]
        return resolve_numeric_authorities(selections, candidates)

    first_authorities = resolved("150")
    second_authorities = resolved("175")
    assert [item["source_ref_ids"] for item in first_authorities] == [
        item["source_ref_ids"] for item in second_authorities
    ]

    def write(authorities: list[dict], name: str) -> Path:
        charts = [
            {"box_px": box, **authority}
            for box, authority in zip(
                ([90, 90, 780, 620], [1030, 90, 780, 620]),
                authorities,
                strict=True,
            )
        ]
        ...
        write_pptx(manifest, out, tmp_path / f"{name}.json")
        assert validate_pptx.quantitative_chart_readback_violations(out, [manifest]) == []
        return out

    first = write(first_authorities, "first")
    second = write(second_authorities, "second")

    def values(path: Path) -> dict[str, list[float]]:
        return {
            shape.name: list(shape.chart.series[0].values)
            for shape in Presentation(path).slides[0].shapes
            if shape.has_chart
        }

    assert values(first) == {"chart:revenue": [120.0, 150.0], "chart:profit": [30.0, 45.0]}
    assert values(second) == {"chart:revenue": [120.0, 175.0], "chart:profit": [30.0, 45.0]}
