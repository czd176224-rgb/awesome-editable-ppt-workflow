#!/usr/bin/env python3
"""Unified CLI for reconstruct-editable-slide image generation or editing.

The CLI uses the Codex-managed ChatGPT OAuth session and subscription.
Direct API-key billing is intentionally unsupported.
"""
...
Backend:
  Uses Codex-managed ChatGPT OAuth and subscription capacity only.
...
def _disabled_post_codex_image_json(url: str, body: Dict[str, Any], timeout: int) -> Dict[str, Any]:
    _die("Reconstruction image provider is disabled; use a sealed accepted-page reconstruction capability.")
...
def _dependency_hint(package: str, *, upgrade: bool = False) -> str:
    ...
    return (
        "Install the reconstruct-editable-slide runtime so CLI dependencies are available, for example "
        f"{_cli_reinstall_hint()}, "
        f"or install {package} directly in this environment with `{runtime_python} -m pip install {package_arg}`."
    )


def _ensure_codex_oauth(dry_run: bool) -> None:
    if _codex_available():
        return
    if dry_run:
        _warn("Codex OAuth is unavailable; showing a subscription-only dry-run request.")
        return
    _die(
        "Codex-managed ChatGPT authentication is unavailable for editppt image generation.\n"
        "Open Codex and sign in with ChatGPT, then retry."
    )
...
def _run_codex_image(
    *,
    ...
    started = time.time()
    response = _disabled_post_codex_image_json(endpoint_url, body, int(getattr(args, "timeout", DEFAULT_TIMEOUT)))
...
def _generate(args: argparse.Namespace) -> None:
    ...
    if args.dry_run:
        if _run_codex_image(
            ...
            return
        _print_request({"backend": "codex-oauth", "operation": "generate", "outputs": [str(p) for p in output_paths], **payload})
        return
    ...
    _die("Codex-managed image generation is unavailable; sign in to Codex and retry.")
...
def _edit(args: argparse.Namespace) -> None:
    ...
    if args.dry_run:
        ...
        _print_request({"backend": "codex-oauth", "operation": "edit", "outputs": [str(p) for p in output_paths], **payload_preview})
        return
    ...
    _die("Codex-managed image editing is unavailable; sign in to Codex and retry.")
...
def _add_shared_args(
    ...
    *,
    ...
    parser.add_argument("--model", default=_default_model(), choices=[GPT_IMAGE_2_MODEL], help="Installed image model.")
    if include_prompt:
...
def _disabled_main() -> int:
    _die("Standalone reconstruction image generation is disabled.")
    raise AssertionError("unreachable")
...
""",
    ...
    _ensure_codex_oauth(args.dry_run)
