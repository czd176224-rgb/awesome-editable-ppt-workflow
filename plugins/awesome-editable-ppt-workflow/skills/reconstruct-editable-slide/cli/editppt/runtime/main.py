#!/usr/bin/env python3
"""Unified editppt CLI for the reconstruct-editable-slide skill."""
...
def subprocess_env() -> dict[str, str]:
    ...
    env["PYTHONPATH"] = os.pathsep.join(
        value for value in (str(RUNTIME_DIR), str(workflow_scripts), env.get("PYTHONPATH")) if value
    )
    return env
...
def run_script(script_name: str, argv: list[str]) -> int:
    ...
    return subprocess.run(command, env=subprocess_env()).returncode
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    payload = {
        "setup": "ok" if doctor.returncode == 0 else "needs_attention",
...
def cmd_prepare(args: argparse.Namespace) -> int:
    ...
    prepared = subprocess.run(command, text=True, capture_output=True, env=subprocess_env())
    if prepared.stdout:
...
def cmd_reconstruction_edit(args: argparse.Namespace) -> int:
    skills_root = Path(__file__).resolve().parents[4]
    provider_cli = skills_root / "generate-slide-body-image" / "scripts" / "codex_gpt_image.py"
    if not provider_cli.is_file():
        raise SystemExit("sealed reconstruction provider CLI is missing from the installed plugin")
    return subprocess.call([sys.executable, str(provider_cli), "reconstruct-edit",
                            "--request-capability", args.request_capability,
                            "--workflow-project", args.workflow_project])
...
def cmd_record(args: argparse.Namespace) -> int:
    return run_script(
        "record_manifest_page_result.py",
        [args.run, "--page", str(args.page), "--agent-id", args.agent_id],
    )
...
def cmd_reset(args: argparse.Namespace) -> int:
    ...
    if args.for_repair:
        argv.append("--for-repair")
    return run_script("reset_page_job.py", argv)


def cmd_replace_source(args: argparse.Namespace) -> int:
    return run_script(
        "replace_page_source.py",
        [args.run, "--page", args.page, "--image", args.image],
    )
...
def cmd_finalize(args: argparse.Namespace) -> int:
    return run_script("finalize_manifest_deck_run.py", [args.run])
...
def build_parser() -> argparse.ArgumentParser:
  ...
  - image uses the installed Codex OAuth capability and processes image files.
...
The public workflow uses the installed Codex OAuth image capability.
""",
        formatter_class=HELP_FORMATTER,
        epilog="""Examples:
...
""",
    ...
    backend.add_argument("--mode", choices=["editppt-image-cli"], default="editppt-image-cli", help="Installed Codex OAuth image backend contract.")
    ...
    backend.add_argument("--model", metavar="MODEL", help="Image model label recorded in the installed backend contract.")
    ...
    dispatch = run_sub.add_parser(
        ...
        description="Mark a page as dispatched after a Codex page worker has been created.",
        formatter_class=HELP_FORMATTER,
    )
    ...
    record = run_sub.add_parser(
        "record",
        help="Validate and record one reconstructed page.",
        description="Validate required manifest artifacts and record the manifest as final assembly authority.",
        formatter_class=HELP_FORMATTER,
    )
    record.add_argument("run", metavar="RUN", help="Prepared manifest run directory.")
    record.add_argument("--page", required=True, metavar="PAGE", help="Page id such as page_001.")
    ...
    reset = run_sub.add_parser(
        ...
        description="Return a recorded page, an explicitly inactive/lost dispatched page, or an accepted page explicitly reopened for QA repair to pending.",
        formatter_class=HELP_FORMATTER,
    )
    ...
    reset.add_argument("--for-repair", action="store_true", help="Reopen an accepted page only for a targeted final-QA repair.")
    reset.set_defaults(func=cmd_reset)

    replace_source = run_sub.add_parser(
        "replace-source",
        help="Replace a reset page's accepted visual authority.",
        description="Copy an exact 1904x896 accepted body design into a pending page and update source geometry.",
    )
    replace_source.add_argument("run", metavar="RUN")
    replace_source.add_argument("--page", required=True, metavar="PAGE")
    replace_source.add_argument("--image", required=True, metavar="FILE")
    replace_source.set_defaults(func=cmd_replace_source)
...
""",
    ...
    page = sub.add_parser(
        "page",
        help="Deterministic page-local tools: manifest build, contact sheet, validation.",
...
Use generate/edit through Codex OAuth. Use process-sheet for deterministic
...
""",
    ...
    sealed_edit = image_sub.add_parser(
        "reconstruct-edit", help="Run the sealed accepted-page reconstruction image edit.")
    sealed_edit.add_argument("--request-capability", required=True)
    sealed_edit.add_argument("--workflow-project", required=True)
    sealed_edit.set_defaults(func=cmd_reconstruction_edit)
...
def main() -> int:
    ...
    if len(raw_argv) >= 2 and raw_argv[0] == "image" and raw_argv[1] in {"generate", "edit"}:
        raise SystemExit("editppt image generate/edit is disabled; reconstruction image edits require a sealed accepted-page request")
