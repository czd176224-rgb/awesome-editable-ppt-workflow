...
0. source.png is the accepted body's visual authority: the accepted source image owns chart container placement, composition, and style, including container geometry, hierarchy, palette, spacing, visual rhythm, and major decorative elements. Reconstruct those as closely as editable PowerPoint allows. When page_request.json contains page_plan, only its sealed relationship nodes and edges own node identity, relationship direction, and connector endpoints. When page_request.json contains numeric_authority, the sealed numeric authority owns values, units, periods, labels, and quantitative mark size and position for the matching chart present in source.png. The worker may correct those precise relationship and quantitative details, but must not replace the core exhibit or reading path. It must not change rendering_primitive or chart_variant or calculate new metrics. Do not let approximate marks in source.png override sealed quantitative size, position, or labels. Do not read or reinterpret Word, comments, attachments, generation prompts, or reference selections. Apart from sealed page_plan relationship fields and optional numeric_authority, page_request.json supplies runtime geometry and paths only; it is not content authority. A material visual departure from source.png outside those narrow sealed exceptions is a page failure, not creative freedom.
