...
0. source.png is the accepted body's visual authority: the accepted source image owns chart container placement, composition, and style, including container geometry, hierarchy, palette, spacing, visual rhythm, and major decorative elements. Reconstruct those as closely as editable PowerPoint allows. When page_request.json page_plan contains primary_relationship, only that sealed relationship's nodes and edges own node identity, relationship direction, and connector endpoints. A page_plan without primary_relationship supplies no reconstruction authority; ignore its content inventory, context bridges, and image prompt. When page_request.json contains numeric_authorities (or the legacy single-chart numeric_authority), each sealed authority owns values, units, labels, any explicitly sourced period, and quantitative mark size and position for the manifest chart with the same object_id. The worker must copy every authority unchanged; it may correct those precise relationship and quantitative details, but must not replace the core exhibit or reading path. It must not change rendering_primitive or chart_variant, calculate new metrics, merge chart identities, or invent a missing period. Do not let approximate marks in source.png override sealed quantitative size, position, or labels. Do not read or reinterpret Word, comments, attachments, generation prompts, or reference selections. Apart from sealed primary_relationship fields and optional numeric authorities, page_request.json supplies runtime geometry and paths only; it is not content authority. A material visual departure from source.png outside those narrow sealed exceptions is a page failure, not creative freedom.
...
2. Execute the three steps in order: (1) background recognition and repair, (2) foreground asset separation, (3) native element reconstruction. Inspect `source.png` directly. If its text cannot be read reliably, report `failure_code: text_unreadable` and stop; do not call an OCR service or switch reconstruction engines.
...
4. Rebuild native text, shapes, and tables (section 3). Read text directly from `source.png`. Render formulas with `editppt formula render-latex` per section 3.2.
